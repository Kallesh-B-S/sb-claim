...
public class ClaimController {
    ...
    @PostMapping
    public ResponseEntity<CreateUpdateResponse> createClaim(@Valid @RequestBody CreateClaimRequest requestBody) {
        CreateUpdateResponse response = claimService.createClaim(requestBody);
    ...
    public ResponseEntity<Claim> getClaimById(@Valid @PathVariable("id") Integer claimId) {
        Claim response = claimService.getClaimById(claimId);
        return ResponseEntity.ok(response);
    }
